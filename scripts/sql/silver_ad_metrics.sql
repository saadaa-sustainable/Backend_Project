-- Silver layer: silver_ad_metrics -- one row per ad per reporting
-- date range, flattened from raw_dump_meta (object_type='insights',
-- level='ad') JSONB payloads into typed columns.
--
...
CREATE TABLE IF NOT EXISTS silver_ad_metrics (
    id UUID PRIMARY KEY,
    ...
    CONSTRAINT uq_silver_ad_metrics_grain UNIQUE (ad_id, date_start, date_stop)
);

CREATE INDEX IF NOT EXISTS ix_silver_ad_metrics_ad_id ON silver_ad_metrics (ad_id);
CREATE INDEX IF NOT EXISTS ix_silver_ad_metrics_date_start ON silver_ad_metrics (date_start);
CREATE INDEX IF NOT EXISTS ix_silver_ad_metrics_source_batch_id ON silver_ad_metrics (source_batch_id);
