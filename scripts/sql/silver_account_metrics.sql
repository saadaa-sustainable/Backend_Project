-- Silver layer: silver_account_metrics -- one row per account per reporting
-- date range, flattened from raw_dump_meta (object_type='insights',
-- level='account') JSONB payloads into typed columns.
--
...
CREATE TABLE IF NOT EXISTS silver_account_metrics (
    id UUID PRIMARY KEY,
    ...
    CONSTRAINT uq_silver_account_metrics_grain UNIQUE (account_id, date_start, date_stop)
);

CREATE INDEX IF NOT EXISTS ix_silver_account_metrics_account_id ON silver_account_metrics (account_id);
CREATE INDEX IF NOT EXISTS ix_silver_account_metrics_date_start ON silver_account_metrics (date_start);
CREATE INDEX IF NOT EXISTS ix_silver_account_metrics_source_batch_id ON silver_account_metrics (source_batch_id);
