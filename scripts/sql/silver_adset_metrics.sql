-- Silver layer: silver_adset_metrics -- one row per ad set per reporting
-- date range, flattened from raw_dump_meta (object_type='insights',
-- level='adset') JSONB payloads into typed columns.
--
...
CREATE TABLE IF NOT EXISTS silver_adset_metrics (
    id UUID PRIMARY KEY,
    ...
    CONSTRAINT uq_silver_adset_metrics_grain UNIQUE (adset_id, date_start, date_stop)
);

CREATE INDEX IF NOT EXISTS ix_silver_adset_metrics_adset_id ON silver_adset_metrics (adset_id);
CREATE INDEX IF NOT EXISTS ix_silver_adset_metrics_date_start ON silver_adset_metrics (date_start);
CREATE INDEX IF NOT EXISTS ix_silver_adset_metrics_source_batch_id ON silver_adset_metrics (source_batch_id);
