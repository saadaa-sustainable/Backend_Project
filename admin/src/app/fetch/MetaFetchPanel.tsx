...
// Display order matches the fixed execution order the backend always uses
// (META_LEVEL_FETCH_ORDER) -- ad -> adset -> campaign -> account, an
// explicit policy ("the flow of fetching meta ads related data"), not
// something the checkboxes reorder.
const LEVEL_OPTIONS: { value: MetaInsightsLevel; label: string }[] = [
  { value: "ad", label: "Ads" },
  { value: "adset", label: "Ad Sets" },
  { value: "campaign", label: "Campaigns" },
  { value: "account", label: "Account" },
];
const DEFAULT_LEVELS: MetaInsightsLevel[] = ["ad", "adset", "campaign"];

function LevelSelector({
  selected,
  onToggle,
}: {
  selected: MetaInsightsLevel[];
  onToggle: (level: MetaInsightsLevel) => void;
}) {
  return (
    <div className="mt-3">
      ...
      <div className="flex flex-wrap gap-3">
        {LEVEL_OPTIONS.map((opt) => (
          <label key={opt.value} className="flex items-center gap-1.5 text-xs text-slate-700">
            <input
              type="checkbox"
      ...
      <p className="mt-1 text-[11px] text-slate-400">
        Always runs in this order — Ads → Ad Sets → Campaigns → Account — regardless of which
        boxes are checked.
      </p>
    </div>
  );
...
export function MetaFetchPanel() {
  ...
  const [selectedLevels, setSelectedLevels] = useState<MetaInsightsLevel[]>(DEFAULT_LEVELS);

  function toggleLevel(level: MetaInsightsLevel) {
  ...
  async function handleCreateTable() {
    ...
    setCreateError(null);
    try {
      const res = await createRawTable(name, "meta");
  ...
  async function handleStartFetch(targetTable: string) {
    ...
    try {
      const run = await triggerIngest({
        ...
        meta_levels: selectedLevels.length > 0 ? selectedLevels : undefined,
      });
  ...
  const metaStatus = status?.sources.meta;
  const isRunning = metaStatus?.status === "running";
  ...
  return (
    <div className="flex flex-col gap-5 rounded-md border border-slate-200 bg-white p-5">
      <div className="flex items-center gap-2">
        <SourceLogo source="meta" className="h-5 w-5 shrink-0" />
        <h3 className="text-sm font-medium text-slate-800">Meta Ads</h3>
      </div>
        ...
        <div>
          ...
          {selected && (
            <p className="mt-1 text-xs text-slate-500">
              Last updated: {formatDateTime(selected.last_updated)} — leave the dates below blank
              to resume automatically from the last date this table's insights cover.
            </p>
          )}
          ...
          <LevelSelector selected={selectedLevels} onToggle={toggleLevel} />
          ...
          {createdTable && (
            <>
              <div className="mt-3 grid grid-cols-2 gap-3 max-w-sm">
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-600">
                    Start date (required — Meta has no auto-discovery)
                  </label>
                  <input
                    type="date"
                    value={dateStart}
        ...
        <div className="rounded-md border border-slate-200 px-3 py-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <div className="flex items-center gap-3 text-xs">
              {metaStatus.rows_ingested !== null && (
                <span className="text-slate-500">{metaStatus.rows_ingested} records fetched</span>
              )}
              {metaStatus.error && <span className="text-red-700">{metaStatus.error}</span>}
              <span
                className={`rounded-full px-2 py-0.5 font-medium ${
                  metaStatus.status === "succeeded"
                    ? "bg-emerald-100 text-emerald-700"
                    : metaStatus.status === "failed"
                      ? "bg-red-100 text-red-700"
                      : metaStatus.status === "stopped"
          ...
          <div className="mt-2">
            <ProgressBar status={metaStatus.status} />
          </div>
          <LevelBadges levels={metaStatus.levels} />
        </div>
      )}
    </div>
  );
